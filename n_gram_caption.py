...
import argparse 
import util
from n_gram_sampler import Sampler
...
class CaptionConditionalSampler(Sampler):
    ...
    def forward_backward_from_x_to_condition(self, net, end, image, condition, scorer, n_gram):
        '''
        ...
        for idx, word in enumerate(sentence):
            ...
            # Write caption for each iteration
            num_list = np.argmax(self.lstm.blobs["predict"].data,axis=2)
            numbers.append(num_list[0][0])
        ...
        # Convert to words
        pred = util.convert_numbers_into_words(settings.vocab_file, numbers)
        gt = util.convert_numbers_into_words(settings.vocab_file, sentence)
        
        # Convert to metrics format and compute bleu score
        pred_dict = {'1':[{'caption': ' '.join(pred)}]}
        gt_dict = {'1':[{'caption': ' '.join(gt)}]}
        
        n_gram_score = scorer.score(gt_dict, pred_dict, gt_dict.keys(), n_gram)[n_gram]
        ...
        dst.diff[...] = n_gram_score*grad_sum[0]
...
def main():
    ...
    parser.add_argument('--n_gram_metric', metavar='b', type=str, help='N-gram to condition on')
    ...
    output_image, list_samples = sampler.sampling(condition_net=net, image_encoder=encoder, image_generator=generator, 
                        ...
                        reset_every=args.reset_every, save_every=args.save_every, n_gram=args.n_gram_metric)

    # Save the final image
    util.save_image(output_image, 'n_gram_caption.jpg')
