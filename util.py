...
def convert_words_into_numbers(vocab_file, words):
    ...
    numbers = [ lines.index(w) + 1 for w in words if w in lines]
    numbers.append( 0 )     # <unk> 
    return numbers
    
def convert_numbers_into_words(vocab_file, numbers):
    # Load vocabularty
    f = open(vocab_file, 'r')
    lines = f.read().splitlines()
    words = []
    for num in numbers:
        if num !=0:
            words.append(lines[num-1])
        else: words.append(lines[num])
